...
estout 	rob_cmove_clrsma* using "output\tables\table_06a.txt", ///
		replace $rename $refcat $semitop $semibottom $keep $labels 

estout 	nofmp_cinter_clrsma* using "output\tables\table_06b.txt", ///
		replace $rename $refcat $semitop $semibottom $keep $labels  

estout 	onlyfmp_cinter_clrsma* using "output\tables\table_06c.txt", ///
		replace $rename $refcat $semitop $semibottom $keep $labels  
		
estout 	neg_cinter_clrsma* using "output\tables\table_06d.txt", ///
		replace $rename $refcat $semitop $semibottom $keep $labels 

estout 	nonig_cinter_clrsma* using "output\tables\table_06e.txt", ///
		replace $rename $refcat $semitop $semibottom $keep $labels  
...
estout 	percent_cinter_clrsma* using "output\tables\table_06f.txt", ///
		replace $rename $refcat $semitop $semibottom $keep $labels 
...
foreach var in cinter {
	...
	*Panel G: FMPs open Jan 2018 - Dec 201
	xtnbreg `var' b2.`sma'  $control $fe if cropincell==1 & lrsma12gs!=., ///
	iter(20) fe difficult
...
estout 	jandec_cinter_clrsma* using "output\tables\table_06g.txt", ///
		replace $rename $refcat $semitop $semibottom $keep $labels 
