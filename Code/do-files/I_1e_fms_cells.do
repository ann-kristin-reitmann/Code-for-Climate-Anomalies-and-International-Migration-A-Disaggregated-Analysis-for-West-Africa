...
*-------------------------------------------------------------------------------------------
* 1e. Data prep do-file: match FMS data to raster data 
*-------------------------------------------------------------------------------------------
...
*This do-file joins FMS data with the cell id 
...
local path `data' 
	...
	save "data\FMS_2018_2019_cells.dta", replace

	preserve 
	keep if largecity==1 // keep urban migration 
	save "data\FMS_2018_2019_urban.dta", replace
	restore 

	preserve
	keep if largecity==0 // keep rural migration 
	save "data\FMS_2018_2019_rural.dta", replace
	restore 
	}
	
	if  "`path'" == "matched_5x5" {

	save "data\FMS_2018_2019_cells_5x5.dta", replace

	preserve 
	keep if largecity==1 // keep urban migration 
	save "data\FMS_2018_2019_urban_5x5.dta", replace
	restore 

	preserve
	keep if largecity==0 // keep rural migration 
	save "data\FMS_2018_2019_rural_5x5.dta", replace
	restore 
	}
}
