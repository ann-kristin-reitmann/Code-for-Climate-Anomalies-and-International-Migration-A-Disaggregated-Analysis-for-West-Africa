...
***indicator to restrict sample 
	...
	areg qdmigeurope `lag`i''  $control $fe if cropincell==1 & flag3==1, ///
	absorb(_ID) $se
	eststo qeurope_lag`i'_crop
...
***indicator to restrict sample  
	...
	forvalues i=0(1)3 {
  	...
  	xtnbreg   qdmigeurope `lag`i''  $control $fe if cropincell==1 & xflag8==1, ///
	iter(20) fe difficult
	eststo  xqeuro_lag`i'_crop
...
estout 	qeurope_lag0_crop qeurope_lag1_crop qeurope_lag2_crop qeurope_lag3_crop ///
		using "output\tables\table_A6a.txt", ///
		replace $rename $refcat $semitop $semibottom keep(*lrsma) drop(2*) $labels 

estout 	xqeuro_lag0_crop  xqeuro_lag1_crop xqeuro_lag2_crop xqeuro_lag3_crop ///
		using "output\tables\table_A6b.txt", ///
		replace $rename $refcat $semitop $semibottom keep(*lrsma) drop(2*) $labels 
